...
// Minimal TIFF decoder for IIP fallback (used when WASM Base64Decoder fails)
function decodeTiffFallback(d: Uint8Array): { width: number; height: number; pixels: Uint8ClampedArray } | null {
    ...
    function lzwDecode(input: Uint8Array, expectedLen: number): Uint8Array | null {
        ...
        while (true) {
            ...
            if (c >= tbl.length) {
                if (!prev) return null;
                const ent = new Uint8Array(prev.length + 1);
                ent.set(prev); ent[prev.length] = prev[0];
                tbl.push(ent);
            ...
            const ent = tbl[c];
            if (prev) {
                const merged = new Uint8Array(prev.length + 1);
                merged.set(prev); merged[prev.length] = ent[0];
                tbl.push(merged);
                if (nc === (1 << cs) && cs < 12) cs++; nc++;
            }
            out.set(ent, op); op += ent.length; prev = ent;
                ...
                iipH.end = function(success: boolean) {
                    ...
                    if (success && headerType === 1) {
                        try {
                            ...
                            if (colonIdx < 0 || colonIdx >= rawBytes.length - 1) return result;
                            ...
                            if (!isTiff) return result;

                            // Decode TIFF
                            const tiffResult = (window as any).__decodeTiffFallback?.(decodedBytes);
                            if (!tiffResult) return result;

                            // Compute target dimensions using addon's _resize
                            const rw = (iipH as any)._resize(tiffResult.width, tiffResult.height);
                            const w = Math.floor(rw[0]);
                            const hh = Math.floor(rw[1]);

                            const canvas = document.createElement("canvas");
                            if (w === tiffResult.width && hh === tiffResult.height) {
                                ...
                                if (ctx) { ctx.imageSmoothingEnabled = true; ctx.drawImage(tmp, 0, 0, w, hh); }
                            }
                            const storage = (iipH as any)._storage;
                            storage.addImage(canvas);
                            return true;
                        } catch (e) {
                            console.error("[IIP-FALLBACK] atob fallback failed:", e);
                        }
                    }
                    return result;
                };
                iipH.__diagPatched = true;
            }

            (window as any).__imageAddon = this.imageAddon;
            (window as any).__term = this.terminal;
            (window as any).__decodeTiffFallback = decodeTiffFallback;

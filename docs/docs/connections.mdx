...
| ssh:localforward | A list of local port forwarding rules. Each entry is either a bare string (`"8080 localhost:80"`) or an object with `rule` (required), `note` (optional description), and `enabled` (optional boolean, defaults to `true`): `{"rule":"8080 localhost:80","note":"Postgres","enabled":true}`. These are **merged** (appended) with the `~/.ssh/config` `LocalForward` values, so rules from both sources are applied. |
| ssh:remoteforward | A list of remote port forwarding rules, using the same string-or-object shape as `ssh:localforward` (`rule`, `note`, `enabled`). These are **merged** (appended) with the `~/.ssh/config` `RemoteForward` values, so rules from both sources are applied. |

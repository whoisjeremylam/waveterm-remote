...
export class WaveBrowserWindow extends BaseWindow {
    ...
    async queueCreateTab(connName?: string) {
        await this._queueActionInternal({ op: "createtab", connName });
    }
    ...
    private async processActionQueue() {
        while (this.actionQueue.length > 0) {
            try {
                ...
                switch (entry.op) {
                    case "createtab":
                        tabId = await WorkspaceService.CreateTab(this.workspaceId, null, true, entry.connName ?? "");
                        break;
                    case "switchtab":
                        tabId = entry.tabId;
...
ipcMain.on("create-tab", async (event, connName?: string) => {
    const senderWc = event.sender;
    const ww = getWaveWindowByWebContentsId(senderWc.id);
    if (ww != null) {
        await ww.queueCreateTab(connName);
    }
